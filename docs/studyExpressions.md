...
Returns the timestamp of the last submission either for any survey or for the specified survey key.

Functional Description:

```
getLastSubmissionDate([surveyKey]): (float64, error)
```

**Parameters:**
> `expression.Data[0]` : the key of the survey as `string`, optional. If not provided, the last (latest) submission date of any survey is returned.

**Note:** The length of `expression.Data` must be `0` or `1`.
