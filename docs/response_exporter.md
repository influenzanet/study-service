...
1. ItemVersion: version number of the respective survey question
    * **Column Name:** *qK* + *sep* + `metaItemVersion`
2. Position: position index of the question in the questionnaire
    * **Column Name:** *qK* + *sep* + `metaPosition`
3. Initialised: timestamp when item was added to survey tree which means the date when condition for displaying the specified item was fulfilled
    * **Column Name:** *qK* + *sep* + `metaInit`
4. Displayed: timestamp when item was displayed on the desktop
    * **Column Name:** *qK* + *sep* + `metaDisplayed`
5. Responded: timestamp when answer was given for the specified question
